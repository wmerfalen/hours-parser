...
export class HoursParser {
  ...
  parse(s: string) :  ParseStatus {
     ...
     */
    let matches : any = this.input.match(/^([0-9]{1,2})(am|pm)-([0-9]{1,2})(am|pm)$/i);
    this.matchedAt = 0;
    ++this.matchedAt; // 1
    if(matches){
      this.times = new MultipleTimeRanges(
        new TimeRange(
          ...
          new Time(Number(matches[3]),0,matches[4])
        )
      );
      return 'ok';
    }
    ...
    if(matches){
      ...
      this.times = new MultipleTimeRanges(
        new TimeRange(
          new Time(Number(matches[1]),0,shift),
          new Time(Number(matches[2]),0,matches[3])
        )
      );
      return 'ok';
    }
    ...
    if(matches) {
      this.times = new MultipleTimeRanges([
        ...
        new TimeRange(
          new Time(Number(matches[6]),Number(matches[7]),matches[8]),
          new Time(Number(matches[9]),Number(matches[10]),matches[11]),
        )
      ]);
      return 'ok';
