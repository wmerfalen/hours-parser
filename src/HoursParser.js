    ...
    constructor(r) {
        ...
        if (typeof r === 'string') {
            ...
            if (r.toLocaleLowerCase() === 'open 24 hours') {
                this.times = [
                    new TimeRange(new Time(12, 0, 'am'), new Time(12, 0, 'am'))
                ];
            }
        }
        if (Array.isArray(r)) {
...
class HoursParser {
    ...
    parse(s) {
         ...
         */
        let matches = this.input.match(/^([0-9]{1,2})(am|pm)-([0-9]{1,2})(am|pm)$/i);
        this.matchedAt = 0;
        ++this.matchedAt; // 1
        if (matches) {
         ...
         */
        matches = this.input.match(/^([0-9]{1,2})\s?-\s?([0-9]{1,2})(am|pm)\s?$/i);
        ++this.matchedAt; // 4
        if (matches) {
            const startHour = Number(matches[1]);
            let shift = 'pm';
         ...
         */
        matches = this.input.match(/^([0-9]{1,2}):([0-9]{1,2})\s?-\s?([0-9]{1,2}):([0-9]{1,2})(am|pm)\s?$/i);
        ++this.matchedAt; // 5
        if (matches) {
            const startHour = Number(matches[1]);
            let shift = 'pm';
            ...
            this.times = new MultipleTimeRanges(new TimeRange(new Time(Number(matches[1]), Number(matches[2]), shift), new Time(Number(matches[3]), Number(matches[4]), matches[5])));
            return 'ok';
        }
        /**
         * Check for
         * NN:NN-NN:NN[AM|PM]
         */
        matches = this.input.match(/^([0-9]{1,2}):([0-9]{1,2})\s?-\s?([0-9]{1,2}):([0-9]{1,2})\s?(am|pm)\s?$/i);
        ++this.matchedAt; // 6
        if (matches) {
            let shift = 'pm';
         ...
         * 10 => PM
         */
        matches = this.input.match(/^([0-9]{1,2}):([0-9]{1,2})\s?-\s?([0-9]{1,2}):([0-9]{1,2})\s?(am|pm)\s?,\s?([0-9]{1,2}):([0-9]{1,2})\s?-\s?([0-9]{1,2}):([0-9]{1,2})\s?(am|pm)\s?$/i);
        ++this.matchedAt; // 7
        if (matches) {
            let shift = 'pm';
            if (Number(matches[1]) < 12) {
        ...
        ++this.matchedAt; // 10
        if (this.input.match(/24 hours/i)) {
         ...
         * Check for
         * NN:NN-NN:NN[AM|PM], NN:NN-NN:NN[AM|PM]
         * i.e.: "12:00 – 2:00 PM, 4:30 PM – 10:00 PM",
         *
         * matches[1] = 12
         * 2 => 00
         * 3 => 2
         ...
         * 11 => PM
         */
        matches = this.input.match(/^([0-9]{1,2}):([0-9]{1,2})\s?-\s?([0-9]{1,2}):([0-9]{1,2})\s?(am|pm)\s?,\s?([0-9]{1,2}):([0-9]{1,2})\s?(am|pm)\s?-\s?([0-9]{1,2}):([0-9]{1,2})\s?(am|pm)\s?$/i);
        ++this.matchedAt; // 12
        if (matches) {
            this.times = new MultipleTimeRanges([
                new TimeRange(new Time(Number(matches[1]), Number(matches[2]), matches[5]), new Time(Number(matches[3]), Number(matches[4]), matches[5])),
                new TimeRange(new Time(Number(matches[6]), Number(matches[7]), matches[8]), new Time(Number(matches[9]), Number(matches[10]), matches[11]))
            ]);
            return 'ok';
        }
        /**
        ...
        if (parts.length === 3) {
            ...
            parts.forEach((value) => {
                ...
                if (matches) {
                     ...
                     */
                    let shift = 'pm';
                    if (Number(matches[1]) < 12 && Number(matches[4]) < 12 && matches[5].toLocaleLowerCase() === 'am') {
                        shift = 'am';
                    }
                    else if (Number(matches[1]) >= 12 && matches[5].toLocaleLowerCase() === 'pm') {
                        shift = 'pm';
                    }
                    let stopShift = matches[5].toLocaleLowerCase() === 'am' ? 'am' : 'pm';
                    ranges.push(new TimeRange(new Time(Number(matches[1]), Number(matches[2]), shift), new Time(Number(matches[3]), Number(matches[4]), stopShift)));
                    return;
                }
                /**
                 * Match against
